...
// formatConsoleLine - форматирование строки для вывода в консоль (с цветами и выравниванием)
func formatConsoleLine(time_now, type_input, text_input string, err error) string {
	reset := "\033[0m"
	var color string
	switch type_input {
	case "INFO":
	...
	if err == nil {
		return fmt.Sprintf("%s %s[%s]%s %s\n", time_now, color, type_input, reset, text_input)
	}
	return fmt.Sprintf("%s %s[%s]%s %s Error: %v.\n", time_now, color, type_input, reset, text_input, err)
}
...
func loggingConsole(text_input, type_input string, err error) {
	time_now := time.Now().UTC().Format(time.DateTime)
	consoleLine := formatConsoleLine(time_now, type_input, text_input, err)
	unix.Write(1, []byte(consoleLine))
}
...
func loggingJson(c *Collector, strct any, title string, flag bool, file int) {
	loggingFile(c, fmt.Sprintf("Starting to write JSON structure \"%s\".", title), "INFO", nil)

	// Маршалим структуру в JSON
	data, err := json.Marshal(strct)
	data = append(data, '\n')
	if err != nil {
		text_input := fmt.Sprintf("Failed to convert \"%v\" to JSON.", title)
		loggingFilePlusConsole(c, text_input, "ERROR", err)
		return
	}
	...
	text_input := fmt.Sprintf("Finished writing JSON structure \"%v\".", title)
	text_input_err := fmt.Sprintf("Failed to write JSON structure \"%v\".", title)
	if flag {
		if err == nil {
			loggingFilePlusConsole(c, text_input, "INFO", err)
		} else {
			loggingFilePlusConsole(c, text_input_err, "ERROR", err)
		}
		...
		if err == nil {
			...
			loggingFile(c, text_input_err, "ERROR", err)
		}
	}
}
